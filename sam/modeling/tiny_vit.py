...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        self.key_dict = {1: 'res2', 2: 'res3', 3: 'res4'} # LBK EDIT
    ...
    def forward_features(self, x):
        ...
        x_dict = {}
        for idx, f in enumerate(self.layers):
            x = f(x)
            if idx in self.key_dict.keys():
                x_dict[self.key_dict[idx]] = x.view(x.shape[0], 
                                                    int(math.sqrt(x.shape[1])),
                                                    int(math.sqrt(x.shape[1])),
                                                    x.shape[2]).permute(0, 3, 1, 2) # LBK
        B,L,C=x.size()
